...
(function ($) {
	var methods = {
		...
		"getClassName": function (key, value, storage) {
			if (!storage)
				storage = <FlexiTableStorage>$(this).data("storage");
	...
	var defaultOptions = {
		...
		onRequestData: null, // function onRequestData(singleRowData): any
		onSort: null, // function onSort(key, values): []
		onChange: null, // function onChange(rows, cols)
		onRenderCell: null, // function onRenderCell($cell, multipleRowData)
		onRenderHeader: null, // function onRenderHeader(key, value): string
		onRenderField: null, // function onRenderField(fieldName): string
	...
	$.fn.FlexiTable.prepareTree = function (options, keys, storage: FlexiTableStorage):TreeNode {
	...
	$.fn.FlexiTable.getClassesFromNode = function (node: TreeNode, options, storage: FlexiTableStorage): string[]{
		var colClasses = [];
		var current = node;
	...
	$.fn.FlexiTable.renderFlexiTable = function (options, storage) {
		...
		var colClassIndexLookUp: string[] = [];
		var lastLevelNumber: number = colLevels.length - 1;
